...
import {hasDiscreteDomain, Scale, SCALE_CATEGORY_INDEX} from './scale';
...
export interface BandMixins {
  /**
   * Relative position on a band of a stacked, binned, time unit or band scale. If set to `0`, the marks will be positioned at the beginning of the band. If set to `0.5`, the marks will be positioned in the middle of the band.
   *
   * @minimum 0
   * @maximum 1
   */
...
export function getBandSize({
  ...
  if (hasDiscreteDomain(scaleType) && isRectBasedMark(mark.type)) {
...
export function hasBandEnd(fieldDef: FieldDef<string>): boolean {
